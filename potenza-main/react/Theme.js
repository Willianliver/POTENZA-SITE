...
export default function Theme() {
  useEffect(() => {
    if (canUseDOM) {
      console.log(
        '%c ' + manifest.name + '@' + manifest.version + ' theme.js is running',
        'color: ' +
          style.semanticColors['active-background'].success +
          '; font-size: 20px;'
      )
        ...
        fetch(url)
          ...
          .then((data) => {
            if (data) {
              ...
              updateData(url, data).then((result) => {
                fetch(url)
                  .then((response) => response.json())
                  .then((data) => {
      ...
      try {
        const intervalFiltrosTitle = setInterval(() => {
          let element = document.querySelector(
            '.vtex-search-result-3-x-filterSelected span'
          )
          if (element) {
