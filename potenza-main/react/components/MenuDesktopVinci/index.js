...
export default function MenuDesktopVinci({ titulo, ofertas }) {
  ...
  let numberOfItens = Math.floor(
    window.innerWidth / 110 - window.innerWidth / 200
  )
  ...
  return (
    <div className={styles.customMenu} onMouseLeave={closeSubmenu}>
      <nav className={styles.MenuDesktopVinci}>
          ...
          <ul className={styles.MenuDesktopVinci__ul}>
            ...
            >
              <svg
                ...
                width="24.999"
                height="18"
                viewBox="0 0 24.999 18"
              >
                <path
                  class="a"
                  d="M7,105a1,1,0,0,1-1-1v-2a1,1,0,0,1,1-1H30a1,1,0,0,1,1,1v2a1,1,0,0,1-1,1Zm0-7a1,1,0,0,1-1-1V95a1,1,0,0,1,1-1H30a1,1,0,0,1,1,1v2a1,1,0,0,1-1,1Zm0-7a1,1,0,0,1-1-1V88a1,1,0,0,1,1-1H30a1,1,0,0,1,1,1v2a1,1,0,0,1-1,1Z"
        ...
        <div className={styles.categoriasContainer}>
          <ul
            className={
              categories.length >= 8
                ? styles.MenuDesktopVinci__ul
                : styles.MenuDesktopVinci__ul__minimal
            }
          >
            {categories.slice(0, numberOfItens).map((cat, index) => {
              return (
                <li className={styles.MenuDesktopVinci__ul_li} key={index}>
                  <a
                    className={styles.MenuDesktopVinci__ul_li_a}
                    href={cat.url}
                  >
                    {cat.name}
                  </a>
      ...
      <nav
        className={`${styles.SubmenuDesktopVinci}`}
                ...
                <ul className={styles.SubmenuDesktopVinci__ul}>
                  {chunk.map((cat, j) => {
                    return (
                      <li key={j}>
                        <a
                          className={styles.SubmenuDesktopVinci__ul_li_a}
                          href={cat.url}
                        >
                          {cat.name}
                        </a>
