...
const ProductSpecificationGroup: FC<ProductSpecificationGroupProps> = ({
  ...
  try {
    ...
    groups.forEach((group) => {
      if (
        slugify(group.name, { lower: true }) == 'tabela-nutricional' ||
        slugify(group.name, { lower: true }) == 'informacao-nutricional'
      ) {
    ...
    return (
      <div
        className={`specificationCustom vtex-store-components-3-x-container ph3 ph5-m ph2-xl mw9 center`}
      >
        <div>
          <Tabs>
            {groups.map((group, index) => (
              ...
              >
                {(() => {
                  if (
                    slugify(group.name, { lower: true }) != 'receita' &&
                    slugify(group.name, { lower: true }) != 'receitas'
                  ) {
                    return (
                      <table className={styles.specificationsTable}>
                        {group.specifications.map((specification) => {
                          if (
                            slugify(specification.name, { lower: true }) ==
                            'header'
                          ) {
                            return (
                              <thead className={styles.specificationRow}>
                                <tr>
                                  {specification.values.map((value) => (
                                    <>
                                      <th
                                        className={styles.specificationValue}
                                      ></th>
                                      {value
                                        .split('-')
                              ...
                              <tr className={styles.specificationRow}>
                                ...
                                'observacao' ? (
                                  ...
                                  specification.values.map((value: any) => (
                                    <>
                                      {slugify(group.name, { lower: true }) ==
                                        'tabela-nutricional' ||
                                      slugify(group.name, { lower: true }) ==
                                        'informacao-nutricional'
                                        ? fakeColumns.map(
                                            (fakeIndex: any, colIndex: any) => (
                                              <td key={index + colIndex}>
                                                {value.split(' - ')[fakeIndex]
                                                  ? value.split(' - ')[
                                                      fakeIndex
                                                    ]
                                                  : '-'}
                    ...
                    if (canUseDOM) {
                      ...
                      let htmlDoc = parser.parseFromString(
                        group.specifications[0].values[0],
                        'text/html'
                      )
                      return (
                        <div
                          dangerouslySetInnerHTML={{
                            __html: htmlDoc.body.innerHTML,
                          }}
                        />
                      )
                    }
