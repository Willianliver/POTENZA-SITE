...
const Breadcrumb: React.FC<Props> = ({
  ...
  return (
    <div
      ...
      className={`${handles.container} ${styles.breadcrumb} pv3`}
    >
      <Link
      ...
      {navigationList.map(({ name, href }, i) => {
        ...
        return (
          <span key={`navigation-item-${i}`}>
            {/* <IconCaret orientation="right" size={caretIconSize} /> */}
            <span className={styles.separator}>{'>'}</span>
            <Link
              className={`${linkBaseClasses} ${
                i == navigationList.length - 1 && !term
                  ? styles.breadPrincipal
                  : ''
              }`}
              to={href}
      ...
      {term && (
        <Fragment>
          ...
          <span className={styles.separator}>{'>'}</span>
          <span
            className={`${handles.term} ph2 c-on-base ${styles.breadPrincipal}`}
          >
            {term}
          </span>
        </Fragment>
